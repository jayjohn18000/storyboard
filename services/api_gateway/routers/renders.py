...
async def get_render(
    ...
    # Call Render Orchestrator service
    try:
        ...
        # Convert response to RenderResponse object
        render_data = response
        render = RenderResponse(
            id=render_data["render_id"],
            timeline_id=render_data.get("timeline_id"),
            storyboard_id=render_data.get("storyboard_id"),
            case_id="unknown",  # Would come from database
            status=RenderStatus(render_data.get("status", "unknown")),
            priority=0,
            created_by=current_user,
            created_at=render_data.get("created_at"),
            updated_at=render_data.get("created_at"),
            started_at=None,
            completed_at=None,
            width=1920,
            height=1080,
            ...
            render_time_ms=render_data.get("render_time_ms", 0),
            checksum="",
            golden_frame_checksums=[]
        )
...
async def get_case_renders(
    ...
    try:
        ...
        response = await http_client.request_json(
            "GET",
            f"{render_url}/cases/{case_id}/renders",
            timeout=10
        )
        
        # Convert response to RenderResponse objects
        renders = []
        for render_data in response.get("renders", []):
            render = RenderResponse(
                id=render_data["render_id"],
                timeline_id=render_data.get("timeline_id"),
                storyboard_id=render_data.get("storyboard_id"),
                case_id=case_id,
                status=RenderStatus(render_data.get("status", "unknown")),
                priority=render_data.get("priority", 0),
                created_by=current_user,
                created_at=render_data.get("created_at"),
                updated_at=render_data.get("created_at"),
                started_at=None,
