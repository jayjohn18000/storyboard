...
class TransitionHandler:
    ...
    def _parse_timeline(self, timeline_data: Dict[str, Any]) -> otio.Timeline:
    ...
    def _parse_track(self, track_data: Dict[str, Any]) -> otio.Track:
        """Parse track from dictionary."""
        try:
            # Create track
            track = otio.Track(
                name=track_data.get("name", "Track"),
                kind=getattr(otio.TrackKind, track_data.get("kind", "Video"))
            )
    ...
    def _parse_clip(self, clip_data: Dict[str, Any]) -> otio.Clip:
    ...
    async def _process_transitions(self, timeline: otio.Timeline) -> otio.Timeline:
    ...
    async def _process_track_transitions(self, track: otio.Track):
    ...
    async def _process_transition_clip(self, track: otio.Track, index: int, transition_clip: otio.Clip):
    ...
    def _is_transition_clip(self, clip: otio.Clip) -> bool:
    ...
    def _validate_transitions(self, timeline: otio.Timeline) -> Dict[str, Any]:
        ...
        try:
            ...
            for track in timeline.tracks:
                for item in track:
                    if isinstance(item, otio.Transition):
            ...
            for track in timeline.tracks:
                transitions = [item for item in track if isinstance(item, otio.Transition)]
    ...
    def _timeline_to_dict(self, timeline: otio.Timeline) -> Dict[str, Any]:
                ...
                "tracks": [
                    {
                        ...
                        "clips": [
                            {
                                "name": item.name,
                                "type": "transition" if isinstance(item, otio.Transition) else "clip",
                                "media_reference": {
    ...
    def _create_fade_transition(self, duration: float) -> otio.Transition:
        """Create fade transition."""
        return otio.Transition(
    ...
    def _create_dissolve_transition(self, duration: float) -> otio.Transition:
    ...
    def _create_wipe_transition(self, duration: float) -> otio.Transition:
    ...
    def _create_slide_transition(self, duration: float) -> otio.Transition:
    ...
    def _create_zoom_transition(self, duration: float) -> otio.Transition:
    ...
    def _create_pan_transition(self, duration: float) -> otio.Transition:
