...
readiness_checker = ReadinessChecker("timeline-compiler")
...
app.add_middleware(RequestContextMiddleware, service_name="timeline-compiler")
...
async def compile_timeline(request: dict):
    ...
    try:
        ...
        # Publish TimelineCompiled event
        try:
            event_bus = await get_event_bus()
            event = EventFactory.create_timeline_compiled(
                timeline_id=timeline_id,
                storyboard_id=storyboard_id or "unknown",
                compilation_results=result
            )
            await event_bus.publish(event)
            logger.info(f"Published TimelineCompiled event for timeline {timeline_id}")
        except Exception as e:
            logger.error(f"Failed to publish TimelineCompiled event: {e}")
            # Don't fail the compilation if event publishing fails
...
if __name__ == "__main__":
    uvicorn.run(
        "services.timeline_compiler.main:app",
